...
class TestStrat () :
    ...
    def buy_ships(self, cp_amount) :
        ship_amounts = {'Scout': 1, 'BattleCruiser': 1, 'Cruiser': 1, 'Destroyer': 1, 'Dreadnaught': 0}
        return ship_amounts
    ...
    def choose_translation(self, ship_info, choices) :
        ...
        if ship_info['name'] == 'Scout' :
            mvmt = (0,0)
            #mvmt = self.to_col(ship_coords,(3,4), choices)
        elif ship_info['name'] == 'BattleCruiser' :
            mvmt = self.to_col(ship_coords,(2,1), choices)
        elif ship_info['name'] == 'Cruiser' :
            mvmt = self.to_col(ship_coords, (4,1), choices)
        elif ship_info['name'] == 'Destroyer' :
            mvmt = self.to_col(ship_coords, (3,5), choices)
